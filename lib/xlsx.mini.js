...
function make_xlsx_lib(XLSX) {
    ...
    var set_ansi = function(cp) { if (VALID_ANSI.indexOf(cp) == -1) return;
    ...
    var CFB = (function _CFB() {
        ...
        */
        ...
        function read_bits_2(buf, bl) { var w = (bl & 7),
                h = (bl >>> 3); return ((buf[h] | (w <= 6 ? 0 : buf[h + 1] << 8)) >>> w) & 0x03; }

        function read_bits_3(buf, bl) { var w = (bl & 7),
                h = (bl >>> 3); return ((buf[h] | (w <= 5 ? 0 : buf[h + 1] << 8)) >>> w) & 0x07; }

        function read_bits_4(buf, bl) { var w = (bl & 7),
                h = (bl >>> 3); return ((buf[h] | (w <= 4 ? 0 : buf[h + 1] << 8)) >>> w) & 0x0F; }

        function read_bits_5(buf, bl) { var w = (bl & 7),
                h = (bl >>> 3); return ((buf[h] | (w <= 3 ? 0 : buf[h + 1] << 8)) >>> w) & 0x1F; }

        function read_bits_7(buf, bl) { var w = (bl & 7),
                h = (bl >>> 3); return ((buf[h] | (w <= 1 ? 0 : buf[h + 1] << 8)) >>> w) & 0x7F; }
        ...
        function inflate(data, usz) {
            ...
            while ((header & 1) == 0) {
                ...
                if ((header >>> 1) == 0) {
                    ...
                    boff += 32;
                    /* push sz bytes */
                    if (!usz && OL < woff + sz) { outbuf = realloc(outbuf, woff + sz);
                        OL = outbuf.length; }
                    if (typeof data.copy === 'function') {
                        ...
                        while (sz-- > 0) { outbuf[woff++] = data[boff >>> 3];
                            boff += 8; }
                    continue;
                } else if ((header >>> 1) == 1) {
                    /* Fixed Huffman */
                ...
                if (!usz && (OL < woff + 32767)) { outbuf = realloc(outbuf, woff + 32767);
                    OL = outbuf.length; }
        ...
        function parse_local_file(blob, csz, usz, o, EF) {
            ...
            if (flags & 8) {
                ...
                if (crc32 == 0x08074b50) { crc32 = blob.read_shift(4);
                    wrn = true; }
    ...
    var __toBuffer = function(bufs) { var x = [],
            w = 10240; for (var i = 0; i < bufs[0].length; ++i)
            if (bufs[0][i])
    ...
    function buf_array() {
        ...
        var push = function ba_push(buf) { endbuf();
            curbuf = buf; if (curbuf.l == null) curbuf.l = curbuf.length;
            next(blksz); };
	...
	*/
    ...
    var rc_to_a1 = (function() {
        ...
        function rcfunc($$, $1, $2, $3) {
            ...
            else if ($2.charAt(0) == "[") { rRel = true;
                $2 = $2.slice(1, -1); }

            if ($3.length == 0) cRel = true;
            else if ($3.charAt(0) == "[") { cRel = true;
                $3 = $3.slice(1, -1); }
    ...
    function write_ws_xml_autofilter(data, ws, wb, idx) {
        ...
        if (range.s.r == range.e.r) { range.e.r = decode_range(ws["!ref"]).e.r;
    ...
    function write_ws_xml(idx, opts, wb, rels) {
        ...
        if (o.length > 1) { o[o.length] = ('</worksheet>');
            o[1] = o[1].replace("/>", ">"); }
        return o.join("");
    }
    ...
    */
    ...
    var HTML_ = (function() {
        function html_to_sheet(str, _opts) {
            ...
            for (i = 0; i < rows.length; ++i) {
                ...
                if (hd == "<tr") {++R; if (opts.sheetRows && opts.sheetRows <= R) {--R; break; }
                    C = 0; continue; }
                ...
                for (j = 0; j < cells.length; ++j) {
                    ...
                    for (var midx = 0; midx < merges.length; ++midx) {
                        ...
                        if (_merge.s.c == C && _merge.s.r < R && R <= _merge.e.r) { C = _merge.e.c + 1;
                            midx = -1; }
                    }
                    ...
                    if (opts.dense) { if (!ws[R]) ws[R] = [];
                        ws[R][C] = o; } else ws[encode_cell({ r: R, c: C })] = o;
                    C += CS;
                }
            }
    ...
    function readSync(data, opts) {
        ...
        if (!o.type) o.type = (has_buf && Buffer.isBuffer(data)) ? "buffer" : "base64";
        if (o.type == "file") { o.type = has_buf ? "buffer" : "binary";
            d = read_binary(data); }
        ...
        if (o.type == 'array' && typeof Uint8Array !== 'undefined' && data instanceof Uint8Array && typeof ArrayBuffer !== 'undefined') {
            ...
            // $FlowIgnore
            if (!vu.foo) { o = dup(o);
                o.type = 'array'; return readSync(ab2a(d), o); }
        }
    ...
    function writeSync(wb, opts) {
        ...
        if (o.cellStyles) { o.cellNF = true;
            o.sheetStubs = true; }
        if (o.type == "array") { o.type = "binary"; var out = (writeSync(wb, o));
